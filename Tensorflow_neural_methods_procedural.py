...
#TO FINISH///////////////////////////////////TO WORK WE SHOULD CONVERT THE WHOLE LIBRARY INTO AN OBJECT
def train(data,labels,sess,folds,steps,accuracy,batch_size,summ,acc_summary,iterator):
    validations = []
    for train_index, test_index in KFold(folds).split(data):
        ...
        sess.run(iterator.initializer,feed_dict={X:X_train,Y:Y_train,BATCH_SIZE:10,is_train:True})
        ...
        sess.run(iterator.initializer,feed_dict={X:X_test,Y:Y_test,BATCH_SIZE:X_test.shape[0],is_train:False})
        summary, val_acc = sess.run([acc_summary,accuracy],feed_dict={is_train:False})
        testwriter.add_summary(summary,runs)
        print("Validation accuracy %g"%val_acc) 
        validations.append(val_acc)
    print(validations)
    print("Validation for kfold is: %g"%np.mean(validations))
